    ...
    const Dashboard: React.FC<DashboardProps> = ({ 
        ...
        const handleSaveExpenseReport = async (reportData: Omit<ExpenseReport, 'id'> | ExpenseReport) => {
            let success = false; // <-- ADD THIS
            
            if ('id' in reportData) {
                await onUpdateExpenseReport(reportData as ExpenseReport);
                success = true; // Assume update succeeds (we can wire this up later)
            } else {
                // --- THIS IS THE KEY CHANGE ---
                // Wait for the function and check its return value
                success = await onAddExpenseReport(reportData);
            }
            
            // --- ONLY close the modal if the save worked ---
            if (success) {
