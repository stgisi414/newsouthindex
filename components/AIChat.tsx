...
const AIChat: React.FC<AIChatProps> = ({ onCommandProcessed, isAdmin, currentUser, onAiSearch }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log('[AIChat RAW commandResponse]', JSON.stringify(commandResponse, null, 2)); // <-- ADD THIS

            // Destructure (Make sure this line is corrected as per step 1)
            const { intent, data, responseText } = commandResponse.data;
            console.log('[AIChat Destructured]', { intent, data, responseText }); // <-- ADD THIS

            // Call the processor in App.tsx
            const result = await onCommandProcessed(intent, data);
            console.log('[AIChat onCommandProcessed result]', JSON.stringify(result, null, 2)); // <-- ADD THIS

            let aiResponseText = result.message || responseText || "I'm not sure how to respond to that.";
            console.log('[AIChat Final aiResponseText]', aiResponseText);

            // Check if a specific message was already generated by onCommandProcessed
            const alreadyHandled = !!result.message; // NEW flag
            ...
            if (!alreadyHandled && intent === 'FIND_BOOK' && result.success) {
            ...
            if (!alreadyHandled && intent === 'FIND_EVENT' && result.success) {
            ...
            if (!alreadyHandled && intent === 'METRICS_DATA' && result.success) {
            ...
            const aiMessage: ChatMessage = {
                ...
                sender: 'ai',
                text: aiResponseText, // Uses the variable determined above
