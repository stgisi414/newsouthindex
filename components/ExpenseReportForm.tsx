  ...
  const ExpenseReportForm: React.FC<ExpenseReportFormProps> = ({
    ...
    // Handler for typing in the ID manually
    const handleIdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (isPrintMode) return;
      const typedValue = e.target.value;
      
      // We rely on the user typing the number. 
      // We don't update 'contactId' (the UUID) directly with typed text anymore.
      // Instead, we treat this input as a search field.

      // 1. Find contact by matching Sequential ID
      // This logic allows them to type "12" and find "00012"
      const selectedContact = staffContacts.find(c => {
        if (!c.sequentialId) return false;
        return Number(c.sequentialId) === Number(typedValue);
      });

      if (selectedContact) {
        setContactId(selectedContact.id); // Link the hidden UUID for the database
        setContactName(`${selectedContact.firstName} ${selectedContact.lastName}`);
        if (errors.staff) setErrors(prev => ({ ...prev, staff: undefined }));
      } else {
        // If no match, we don't clear the UUID immediately to avoid flashing, 
        // but you might want to if strict matching is required.
        // For now, we just clear the name to show "Invalid".
        setContactId(''); 
        setContactName(''); 
      }
    };

    // Helper to get the display ID for the current selection
    const currentSequentialId = useMemo(() => {
      if (!contactId) return '';
      const contact = staffContacts.find(c => c.id === contactId);
      return contact?.sequentialId || '';
    }, [contactId, staffContacts]);
                ...
                <div className="flex items-center">
                  ...
                  {!isPrintMode && (
                    <input
                      type="text"
                      placeholder="ID #"
                      // Use a helper to show the ID if a contact is already selected (e.g. on edit)
                      defaultValue={staffContacts.find(c => c.id === contactId)?.sequentialId || ''}
